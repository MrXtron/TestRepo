...
class MovieBoxProvider(private val sharedPref: SharedPreferences? = null) : MainAPI() {
    ...
    companion object {
        val HOST_POOL = listOf(
            "https://api.aoneroom.com",      // 1. Main Global Core API Node
            "https://api.inmoviebox.com",    // 2. Primary India BFF Cluster
            "https://api6.aoneroom.com",     // 3. High-speed India Regional Backup
            "https://api5.aoneroom.com",     // 4. Fallback Asian Gateway
            "https://api4sg.aoneroom.com",   // 5. Singapore Regional Gateway
            "https://api4.aoneroom.com",     // 6. Fallback Mirror 1
            "https://api3.aoneroom.com"      // 7. Fallback Mirror 2 
        )
    ...
    override var name = "MovieBox (Mob)"
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val jsonBody = """{"page":$page,"perPage":$perPage,"channelId":"$channelId","classify":"$classify","country":"$country","year":"$year","genre":"$genre","sort":"$sort"}"""
        ...
        val headers = mapOf(
            "user-agent" to "com.community.mbox.in/50020126 (Linux; U; Android 16; en_IN; sdk_gphone64_x86_64; Build/BP22.250325.006; Cronet/145.0.7582.0)",
            ...
            "x-client-info" to """{"package_name":"com.community.mbox.in","version_name":"4.0.02.0831.03","version_code":50020126,"os":"android","os_version":"16","device_id":"$deviceId","install_store":"ps","gaid":"d7578036d13336cc","brand":"google","model":"${randomBrandModel()}","system_language":"en","net":"NETWORK_WIFI","region":"IN","timezone":"Asia/Calcutta","sp_code":""}""",
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val headers = mapOf(
            "user-agent" to "com.community.mbox.in/50020126 (Linux; U; Android 16; en_IN; ${randomBrandModel()}; Build/BP22.250325.006; Cronet/145.0.7582.0)",
            "accept" to "application/json",
            "content-type" to "application/json",
            "connection" to "keep-alive",
            "x-client-token" to xClientToken,
    ...
    override suspend fun loadLinks(
        ...
        try {
            ...
            val subjectHeaders = mapOf(
                "user-agent" to "com.community.oneroom/50020128 (Linux; U; Android 16; en_US; $brand; Build/TQ3A.230901.001; Cronet/145.0.7582.0)",
                "accept" to "application/json",
                "content-type" to "application/json",
                "connection" to "keep-alive",
                ...
                "X-M-Version" to "4.0.02",
                "x-client-info" to """{"package_name":"com.community.oneroom","version_name":"4.0.02.0903.02","version_code":50020128,"os":"android","os_version":"16","install_ch":"ps","device_id":"$deviceId","install_store":"ps","gaid":"1b2212c1-dadf-43c3-a0c8-bd6ce48ae22d","brand":"$model","model":"$brand","system_language":"en","net":"NETWORK_WIFI","region":"US","timezone":"Asia/Calcutta","sp_code":"","X-Play-Mode":"1","X-Idle-Data":"1","X-Family-Mode":"0","X-Content-Mode":"0"}""".trimIndent(),
                "x-client-status" to "0",
                "Authorization" to "Bearer ${getCachedToken()}"
            )
            ...
            subjectIds.amap { (subjectId, language) ->
                try {
                    ...
                    val headers = mapOf(
                        ...
                        "user-agent" to "com.community.mbox.in/50020150 (Linux; U; Android 16; en_IN; $brand; Build/BP22.250325.006; Cronet/145.0.7582.0)",
                        "accept" to "application/json",
                        "content-type" to "application/json",
                        "connection" to "keep-alive",
                        "x-client-token" to xClientToken,
                        "x-tr-signature" to xTrSignature,
                        "X-M-Version" to "4.1.05",
                        "x-client-info" to """{"package_name":"com.community.mbox.in","version_name":"4.1.05.0112.01","version_code":50020150,"os":"android","os_version":"16","device_id":"$deviceId","install_store":"ps","gaid":"d7578036d13336cc","brand":"google","model":"$model","system_language":"en","net":"NETWORK_WIFI","region":"IN","timezone":"Asia/Calcutta","sp_code":""}""",
                        "x-client-status" to "0",
                        "x-play-mode" to "2"
                    )
                    ...
                    if (response.code == 200) {
                        ...
                        if (streams != null && streams.isArray) {
                            for (stream in streams) {
                                // Dynamic safety layout match to prevent application update cascade
                                val actionStatus = stream["action"]?.asText() ?: ""
                                if (actionStatus.equals("UPDATE_APP", ignoreCase = true)) {
                                    continue // Strict bypass for application version locks
                                }
