...
class DesiSerialsProvider : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        "$currentProxy/?url=$mainUrl/latest-episodes" to "Latest Episodes",
        "$currentProxy/?url=$mainUrl/and-tv" to "& TV",
        "$currentProxy/?url=$mainUrl/color-tv-hd" to "Colors TV",
        "$currentProxy/?url=$mainUrl/sab-tv-hd" to "Sab TV",
        "$currentProxy/?url=$mainUrl/sony-tv" to "Sony TV",
        "$currentProxy/?url=$mainUrl/star-bharat" to "Star Bharat",
        "$currentProxy/?url=$mainUrl/star-plus-hdepisodes" to "Star Plus",
        "$currentProxy/?url=$mainUrl/zee-tv" to "Zee TV"
    )
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        val searchUrl = "$currentProxy/?url=$mainUrl/?s=$encodedQuery"
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val doc = app.get("$currentProxy/?url=$url", headers = mapOf(
            "User-Agent" to "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
            "Accept" to "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8"
        )).document
    ...
    override suspend fun loadLinks(
        ...
        val finalUrl = if (data.startsWith("http")) "$currentProxy/?url=$data" else data
        ...
        if (data.startsWith("http")) {
            ...
            val doc = app.get("$currentProxy/?url=$data", headers = mapOf(
                "User-Agent" to "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
            )).document
