...
class Tvlogyflow(
    ...
    override suspend fun getUrl(
                ...
                if (packedMatch != null) {
                    ...
                    unpacked = Regex("""\b(\w+)\b""")
                        .replace(payload) { match ->
                            ...
                            if (index != null && index < symtab.size) {
                                symtab[index]
                            } else {
                                match.value
                            }
        ...
        for (proxy in XtronPlayTVPlugin.proxy) {
            try {
                val proxyDoc = app.get(
                    "$proxy/?url=$url",
                    referer = mainUrl
                ).text

                if (process(proxyDoc)) return
            } catch (_: Exception) {
                continue
            }
        }
...
class Tvlogy(
    ...
    override suspend fun getUrl(
        ...
        val videoMeta = meta ?: return

        callback(
            newExtractorLink(
                "$name $source",
                name,
                url = videoMeta.videoSource,
